...
def draw_highscorelist(database, screen):
    ...
    for i in range(len(database.scorelist)):
        ...
        dr_no.draw_txt(Stat.infofont, str(
            database.scorelist[i][1]), "white", screen, Stat.screen_width/2+250, height+5)
        height += 60
...
def draw_name_need(database, screen):
    ...
    dr_no.draw_txt(Stat.startfont, "Congrats", "white",
                   screen, Stat.screen_width/2-450, 100)
    dr_no.draw_txt(
        Stat.startfont, f"You survived {database.scorelist[database.ind][1]} days", "white", screen, Stat.screen_width/2-450, 200)
    dr_no.draw_txt(Stat.infofont, "Enter your name:", "white",
                   screen, Stat.screen_width/2-450, 300)
    dr_no.draw_txt(Stat.startfont, str(
        database.scorelist[database.ind][0]), "white", screen, Stat.screen_width/2-450, 400)
    dr_no.draw_txt(Stat.infofont, "press enter to submit name",
                   "white", screen, Stat.screen_width/2-450, 500)
