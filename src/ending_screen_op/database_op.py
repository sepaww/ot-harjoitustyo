...
class Databaseop:
    ...
    def insertintable(self):
        ...
        for i in range(10):
            ...
            self.db.execute("INSERT INTO hs(name,days) VALUES(?,?)", (name, day) )
        self.db.commit()
    ...
    def exist(self):
        ...
        if cur.fetchone()[0]!=1: 
            ...
            self.db.execute("CREATE TABLE hs (name varchar(255), days int)")
            for i in range(10):
               self.db.execute("INSERT INTO hs(name,days) VALUES(?, ?)", ("anon", 0))
               self.db.commit()
    ...
    def geths(self):
        ...
        self.scorelist=[(self.names[i], self.days[i]) for i in range(len(self.days))]
